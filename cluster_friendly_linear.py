...
@profile
def run_quantize_bak(args: dict):
    """
    Static quantize method which can be run in parallel.
    Original implementation without combining the weights for repeated values.
    """
    num_clusters = args["num_clusters"]
    weight = args["weight"]
    sensitivities = args["sensitivities"]
    ...
    while True:
        ...
        weight1d = weight.view(-1).cpu()
        sensitivities1d = sensitivities.view(-1).cpu()

        kmeans = kmeans1d.cluster(weight1d.numpy(), num_clusters, weights=sensitivities1d.numpy() + eps)
        if not np.isnan(kmeans.centroids).any():
            break
        elif eps == 0:
            eps = 1e-12
    ...
    wq = np.array(kmeans.centroids, dtype=np.float32)[np.array(kmeans.clusters)]
    assert np.unique(wq).shape[0] <= num_clusters, f"{np.unique(wq).shape[0]} > {num_clusters} for {name}"
    assert weight1d.shape[0] == wq.shape[0], f"{weight1d.shape[0]} != {wq.shape[0]} for {name}"
